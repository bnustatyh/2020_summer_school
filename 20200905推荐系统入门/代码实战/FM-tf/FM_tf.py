"""
来源：https://github.com/princewen/tensorflow_practice
"""
...
# 设置样本特征，和预测结果
n, p = x_train.shape
x = tf.placeholder('float', [None, p])# 列为p，行不定
...
with tf.Session() as sess:
    sess.run(init)
    for epoch in tqdm(range(epochs), unit='epoch'):# 打印进度条
        perm = np.random.permutation(x_train.shape[0])# shuffle
    ...
    for bX, bY in batcher(x_test, y_test):
        ...
        print(errors)
    RMSE = np.sqrt(np.array(errors).mean())# 均方根误差
    print(RMSE)
    print(w.eval())
    print(v.eval())
    sess.close()